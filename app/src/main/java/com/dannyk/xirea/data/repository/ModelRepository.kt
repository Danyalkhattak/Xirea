...
class ModelRepository(
    ...
    /**
     * Import a model from local storage (e.g., file picker).
     * Copies the file to the app's models directory and creates a DB entry.
     */
    suspend fun importLocalModel(sourceFile: java.io.File): AIModel {
        ...
        val modelId = "local-${System.currentTimeMillis()}"
        val modelName = fileName.removeSuffix(".gguf").replace("-", " ").replace("_", " ")
    ...
    /**
     * Check if a model file already exists in the models directory.
     */
    fun isModelFileExists(fileName: String): Boolean {
        return java.io.File(modelsDir, fileName).exists()
    }
